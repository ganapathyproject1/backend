...
exports.createUser = (req, res, next) => {
    ...
    if(reqValidity.includes(false)) {
        let error = new ErrorResponseBuilder('Invalid request').errorType('DataValidationError').status(400).errorCode('UC-1').build();
        return next(error);
    }
    ...
    bcrypt.hash(req.body.password, 12)
            .then(hash => {
                ...
                .catch(error => {
                    ...
                    let err = new ErrorResponseBuilder().status(500).errorCode('UC-2').errorType('UnknownError').build();
                    return next(err);
                })
            })
}
...
exports.loginUser = (req, res, next) => {
    ...
    User.findOne({ email: req.body.email })
        ...
        .then(result => {
            ...
            const token = jwt.sign({email: fetchedUser.email, firstName: fetchedUser.firstName, lastName: fetchedUser.lastName, id: fetchedUser._id, isAdmin: fetchedUser.hasAdminPrevilieges}, "secret", {expiresIn: '1h'});
            const data = {
                token: token,
                expiryDuration: 3600,
        ...
        .catch(error => {
          console.log(error);
            logger.log(error, req, 'UC-4');
            let err = new ErrorResponseBuilder().status(500).errorCode('UC-4').errorType('UnknownError').build();
            return next(err);
        });
}
    ...
    User.findById(req.userData.userId)
        ...
        .then(result => {
            ...
            Request.find({'requesterId': result._id}, {__v: 0})
                    ...
                    .catch(error => {
                      logger.log(error, req, 'UC-GU');
                      let err = new ErrorResponseBuilder().status(500).errorCode('UC-1').errorType('UnknownError').build();
                      return next(err);
                    });
        })
        .catch(error => {
            logger.log(error, req, 'UC-GU');
            let err = new ErrorResponseBuilder().status(500).errorCode('UC-3').errorType('UnknownError').build();
            return next(err);
        })
}
  ...
  User.findOneAndUpdate({_id: req.userData.userId},
      ...
      .then(docs => {
        if(docs.firstName) {
          ...
          logger.log(docs, req, 'UC-2');
          let err = new ErrorResponseBuilder('Unable to update your details!!! Please try again later...').status(500).errorCode('UC-3').errorType('UnknownError').build();
          return next(err);
        }

      })
      .catch(error => {
        logger.log(error, req, 'UC-3');
        let err = new ErrorResponseBuilder().status(500).errorCode('UC-3').errorType('UnknownError').build();
        return next(err);
      })
}
...
exports.requestUserPassword = (req, res, next) => {
  ...
  if(reqValidity.includes(false)) {
    let error = new ErrorResponseBuilder('Invalid request').errorType('DataValidationError').status(400).errorCode('UC-R-1').build();
    return next(error);
  }
  User.countDocuments({email: req.body.email})
      ...
      .then(docsCount => {
        if(docsCount > 0) {
          ...
          let error = new ErrorResponseBuilder('No user found with the provided email Id, please check again the email provided!!!').status(404).errorType('DataNotFoundError').errorCode('UC-R-2').build();
          return next(error);
        }
      })
      .catch(error => {
        console.log(error);
        logger.log(error, req, 'UC-R-3');
        let err = new ErrorResponseBuilder().status(500).errorCode('UC-R-3').errorType('UnknownError').build(); 
        return next(err);
      })
}
