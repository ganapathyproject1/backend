...
exports.createItem = (req, res, next) => {
  ...
  if(reqValidity.includes(false)) {
    let error = new ErrorResponseBuilder('Invalid request').errorType('DataValidationError').status(400).errorCode('ITEM-ERROR-1').build();
    return next(error);
  }
  let itemObj = {
  ...
  item.save()
      ...
      .catch(error => {
        logger.log(error, req, 'ITEM-ERROR-2');
        let err = new ErrorResponseBuilder().status(500).errorCode('ITEM-ERROR-2').errorType('UnknownError').build();
        return next(err);
      })
}
...
exports.deleteItem = (req, res, next) => {
  ...
  if(reqValidity.includes(false)) {
    let error = new ErrorResponseBuilder('Invalid request').errorType('DataValidationError').status(400).errorCode('ITEM-ERROR-3').build();
    return next(error);
  }
  Item.findOneAndDelete({_id: req.body.id})
      ...
      .catch(error => {
        logger.log(error, req, 'ITEM-ERROR-3');
        let err = new ErrorResponseBuilder().status(500).errorCode('ITEM-ERROR-3').errorType('UnknownError').build();
        return next(err);
      })
}
...
exports.updateItemStatus = (req, res, next) => {
  ...
  if(reqValidity.includes(false)) {
    let error = new ErrorResponseBuilder('Invalid request').errorType('DataValidationError').status(400).errorCode('ITEM-ERROR-4').build();
    return next(error);
  }
  Item.findOneAndUpdate({_id: req.body.id}, {status: req.body.status, 
      ...
      .catch(error => {
        logger.log(error, req, 'ITEM-ERROR-5');
        let err = new ErrorResponseBuilder().status(500).errorCode('ITEM-ERROR-5').errorType('UnknownError').build();
        return next(err);
      });
}
exports.getItemsByListId = (req, res, next) => {
  ...
  if(reqValidity.includes(false)) {
    let error = new ErrorResponseBuilder('Invalid request').errorType('DataValidationError').status(400).errorCode('ITEM-ERROR-6').build();
    return next(error);
  }
  ...
  .catch(error => {
    console.log(error);
    logger.log(error, req, 'ITEM-ERROR-7');
    let err = new ErrorResponseBuilder().status(500).errorCode('ITEM-ERROR-7').errorType('UnknownError').build();
    return next(err);
  });
}
