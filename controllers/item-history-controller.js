...
exports.getLastActivityOnList = (req, res, next) => {
    ...
    if(reqValidity.includes(false)) {
        let error = new ErrorResponseBuilder('Invalid request').errorType('DataValidationError').status(400).errorCode('IH-1').build();
        return next(error);
    }
    ItemHistory.find({listId: req.body.listId, previliegedUsers: req.userData.userId})
                ...
                .catch(error => {
                    logger.log(error, req, 'IH-2');
                    let err = new ErrorResponseBuilder().status(500).errorCode('IH-2').errorType('UnknownError').build();
                    return next(err);
                  })
}