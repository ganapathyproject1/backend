...
exports.createList = (req, res, next) => {
  ...
  if(reqValidity.includes(false)) {
    let error = new ErrorResponseBuilder('Invalid request').errorType('DataValidationError').status(400).errorCode('LE-1').build();
    return next(error);
  }
  const list = new List({
  ...
  list.save()
      ...
      .catch(error => {
        logger.log(error, req, 'LE-2');
        let err = new ErrorResponseBuilder().status(500).errorCode('LE-2').errorType('UnknownError').build();
        return next(err);
      })
}
...
exports.getOwnLists = (req, res, next) => {
  List.find({'owner': req.userData.userId})
      ...
      .catch(error => {
        logger.log(error, req, 'LC-1');
        let err = new ErrorResponseBuilder().status(500).errorCode('LC-1').errorType('UnknownError').build();
        return next(err);
      })
}
...
exports.getFriendsLists = (req, res, next) => {
  if(!req.body.friends || !Array.isArray(req.body.friends)) {
    let error = new ErrorResponseBuilder('Invalid request').errorType('DataValidationError').status(400).errorCode('LC-1').build();
    return next(error);
  }
  ...
  List.find({'owner': {$in:friends}})
      ...
      .then(docs => {
        ...
        User.find({'_id':{$in: friends}})
            ...
            .then(userDocs => {
              ...
              let jsonResponse = new SuccessResponseBuilder('Friends Lists fetched successfully!!!').data(newDocs).build();
              res.status(200).send(jsonResponse);
            })
            .catch(error => {
      ...
      .catch(error => {
        ...
        let err = new ErrorResponseBuilder().status(500).errorCode('LC-3').errorType('UnknownError').build();
        return next(err);
      })
}
...
exports.updateListById = (req, res, next) => {
  ...
  List.findOneAndUpdate({_id: req.body.id},{title: req.body.title, description: req.body.description,lastModifiedOn: new Date().toISOString(), lastModifiedBy: req.body.lastModifiedBy, lastModifiedByName: req.body.lastModifiedByName},{new: true})
      ...
      .catch(error => {
        logger.log(error, req, 'LC-2');
        let err = new ErrorResponseBuilder().status(500).errorCode('LC-2').errorType('UnknownError').build();
        return next(err);
      })
}
